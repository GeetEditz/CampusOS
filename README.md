...
```mermaid
graph TD
    A[Student Profile Onboarding] -->|Profile Context| B(CampusOS AI Opportunity Engine)
    C[Senior Intelligence Feed] -->|Opportunity Context| B
    B -->|API Request| D[NVIDIA NIM Gateway]
    D -->|meta/llama-3.1-70b-instruct| E[Secure LLM Orchestrator]
    E -->|Structured Synthesis| F[Personalized Roadmap]
    E -->|Contextual Counseling| G[Conversational Chat Mentor]
    E -->|Semantic Analysis| H[Opportunity Match Scores]
    
    style B fill:#76B900,stroke:#333,stroke-width:2px,color:#fff
    style D fill:#76B900,stroke:#333,stroke-width:2px,color:#fff
    style E fill:#000,stroke:#76B900,stroke-width:2px,color:#fff
```
...
```mermaid
sequenceDiagram
    participant User as Student / Senior
    participant FE as Next.js 15 App Router (Vercel)
    participant Auth as Supabase Auth (SSO / JWT)
    participant DB as PostgreSQL (Supabase)
    participant NIM as NVIDIA NIM (Llama 3.1)

    User->>FE: Access Platform / Onboard
    FE->>Auth: Validate Credentials / SSO JWT
    Auth-->>FE: Return Auth Token
    FE->>DB: Fetch Profile, Feed Posts, Notifications
    DB-->>FE: Return Relational Datasets
    FE->>NIM: Send Profile Context + Active Opportunities
    NIM-->>FE: Return AI Prepared Roadmaps & Mentorship Guidance
    FE->>User: Render Unified Interactive Interface
```
